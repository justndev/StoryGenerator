...
def createVoice(text: str, uid: str):
...
def delete_temp_files_by_id(uid):
    ...
    paths = [temp_voiced_video_path, temp_voice_path]
    for path in paths:
        try:
            os.remove(path)
...
def generateShortVideo(text: str, video_id, uid):
    result = createVoice(text, uid)
    if result["success"]:
        # Assume video_id is valid and video is existing
        output = f"temp_voiced_videos/{uid}.mp4"
        add_audio_to_video(links[video_id], f"temp_voices/{uid}.mp3", output)
        captacity.add_captions(output, f"temp_ready_videos/{uid}.mp4")
        delete_temp_files_by_id(uid)
    return

def main(text, bg_video_id, uid):
    generateShortVideo(text, bg_video_id, uid)

if __name__ == "__main__":
    text = sys.argv[1]
    bg_video_id = sys.argv[2]
    uid = sys.argv[3]
    main(text, bg_video_id, uid)